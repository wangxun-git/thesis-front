...
<!--            </div>-->
        ...
        <div class="notice">
          <a href="#" v-for="(key,value) in list" @click="changeShow(key.T_FAQ_ID)">
            <i class="el-icon-caret-right"></i>
            <h3>&nbsp;&nbsp;{{key.T_FAQ_TITLE}}
              <p class="date">{{key.T_CREATE.slice(0, 10)}}</p>
            </h3>
          </a>
        </div>
      ...
      methods: {
        ...
        changeShow(faqId) {
          ...
          for (let i = 0; i < this.list.length; i++) {
            if (faqId == this.list[i].T_FAQ_ID) {
              this.drawerTitle = this.list[i].T_FAQ_TITLE
              this.context = this.list[i].T_FAQ_ANSWER
            }
          }
